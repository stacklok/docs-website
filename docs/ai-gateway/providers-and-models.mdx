...
A _provider_ defines an upstream API endpoint and its authentication method.
...
3. Add a route that references the provider. Resource validation requires every
   route to reference a declared provider:
...
Assign the gateway proxy pods an IAM role through IAM roles for service accounts
(IRSA) or EKS Pod Identity. Grant the role the required Bedrock actions.
...
The pricing service resolves OpenRouter rates through the provider name. Use
...
Remove routes that reference the provider, remove the provider, and apply the
resource.
...
- [Route models](./model-routing.mdx) to configure weighting and failover.
- [Budgets and pricing](./budgets-and-pricing.mdx) to publish prices for the
  models you route.
