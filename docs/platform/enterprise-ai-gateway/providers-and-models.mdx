...
5. Send a request through the gateway to confirm the credential works
   end to end:
...
service-account key. The `credentials.region` value must match the region
prefix in the hostname.
...
Service-account keys are the only supported Vertex credential. Workload
Identity Federation is not supported.

### Google AI Studio

Google AI Studio has no native schema of its own. Selecting `GeminiAIStudio`
makes the gateway speak OpenAI's request and response shape and rewrite the
path for AI Studio's OpenAI-compatible surface, so no extra configuration is
needed beyond the API key.
...
can resolve, and its OpenRouter-only model slugs are refused at admission
rather than billed at the wrong rate. Running more than one OpenRouter account,
or renaming the provider, is not supported for spend tracking today.
