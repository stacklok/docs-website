...
`group_variables`. For an inbound SCIM issuer, query a representative user and
inspect the returned document:

```bash title="Inspect a stored SCIM user"
curl --fail --silent --get \
  --header 'Authorization: Bearer <SCIM_BEARER_TOKEN>' \
  --data-urlencode 'filter=userName eq "<USER_NAME>"' \
  'https://<PLATFORM_HOST>/scim/<ISSUER_ID>/v2/Users'
```

For an issuer that uses the outbound Okta reconciler, inspect the target
application's profile mapping in Okta because that issuer has no public SCIM
bearer token. `logRawOktaAPIData` can expose upstream Okta profile responses,
but it does not reliably identify the final stored SCIM path and includes
