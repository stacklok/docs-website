...
Use the telemetry policy to configure an OpenTelemetry collector for Stacklok
client traces and metrics. For a primer on ToolHive's OpenTelemetry support, see
the
...
Use `enforced` to require the central collector. Use `default` to let client
operators override the settings for local debugging or testing.
