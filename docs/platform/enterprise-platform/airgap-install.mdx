...
works unchanged in an air-gapped install. Template the chart directory you
transferred, rather than an `oci://` reference, and pipe it to the
`kubectl-preflight` plugin the same way as the
