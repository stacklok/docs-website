...
See [Configure the Enterprise Manager](../enterprise-manager/configure.mdx) and
...
The per-registry claims drive the console's dropdown: a user sees only the
...
If you'd rather not have the production instance carry the console and the
cross-environment `api` sources, deploy a dedicated Registry Server instance for
aggregation and point the console at it instead. The registry model is the same;
only the instance hosting the aggregating registries changes.
