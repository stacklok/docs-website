...
supports two client registration mechanisms. **Dynamic Client Registration (DCR,
RFC 7591)** is the backward-compatibility fallback — MCP clients call
`/oauth/register` to receive a `client_id` automatically. **Client ID Metadata
Documents (CIMD)**, the MCP specification's preferred mechanism for clients and
servers without a prior relationship, are also supported: a client presents a
public HTTPS URL as its `client_id`, and the embedded AS fetches and validates
the metadata document at that URL. Clients such as VS Code support CIMD and will
use it automatically when the embedded AS advertises support.
