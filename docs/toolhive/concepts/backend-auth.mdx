...
no separate infrastructure needed. It supports Dynamic Client Registration (DCR)
and Client ID Metadata Documents (CIMD), so MCP clients can register or identify
themselves automatically with ToolHive. No manual client configuration in
ToolHive is required.
