...
Stacklok Enterprise compiles RBAC-style custom resources into the Cedar policies
described here, so you can manage authorization across an MCP fleet without
hand-writing a policy per server. See
[enterprise authorization](../../platform/enterprise-authz/index.mdx).
...
Nested map and array claims become Cedar records and sets you can read with dot
access. This is useful for delegation policies that gate on the
[RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693) `act` claim, which
identifies the party acting on behalf of the JWT subject:
...
This policy allows the deploy tool for `user@example.com` when the caller is an
agent from the `agents` namespace acting on the user's behalf. Guard the nested
attribute with `has` before reading it: an absent `act` claim causes an
evaluation error, which ToolHive treats as a deny.

Claims nested more than 10 levels deep are dropped instead of converted, since
claim values can originate from an unverified upstream token. See
[Claim type mapping](../reference/authz-policy-reference.mdx#claim-type-mapping)
...
- Set up authentication and authorization for
  ...
  [Kubernetes-deployed MCP servers](../guides-k8s/authorization-k8s.mdx)
