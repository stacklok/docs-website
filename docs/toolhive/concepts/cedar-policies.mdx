...
Nested map and array claims become Cedar records and sets, so you can read them
with the same dot access shown above (for example `context.claim_act.sub`).
Guard a nested claim with `has` before reading it, since an absent claim causes
an evaluation error, which ToolHive treats as a deny. See
[Claim type mapping](../reference/authz-policy-reference.mdx#claim-type-mapping)
for the full set of conversions, including the nesting depth limit.
...
ToolHive forwards the list request to the MCP server, then automatically filters
the response based on what the caller is authorized to access:
...
- `resources/templates/list` shows only resource templates whose URI template
  the user can read (based on `read_resource` policies)
