...
The embedded auth server's OAuth endpoints (`/oauth/register`,
`/oauth/authorize`, `/oauth/token`, `/oauth/callback`) are always served at the
host root, regardless of any path in `issuer`. Set `issuer` to a bare host, with
no path, even if the resource it protects lives at a path (for example, an MCP
endpoint at `/mcp`). Adding a path to `issuer` makes discovery advertise
...
This also affects the default callback URL: when an upstream provider omits
`redirectUri`, it defaults to `{resourceUrl}/oauth/callback`. That default only
lands somewhere real when `resourceUrl` is also a bare host. If `resourceUrl`
includes a path, set `redirectUri` explicitly to `<issuer>/oauth/callback`
