...
- **Client ID Metadata Documents (CIMD):** When enabled, accepts HTTPS URLs as
  `client_id` values and resolves client metadata on demand. Clients that
  support CIMD (such as VS Code) use it automatically; other clients fall back
  to DCR.
...
sequenceDiagram
    ...
    Note over EmbeddedAS,IDP: Leg 2 — AS's own credentials
