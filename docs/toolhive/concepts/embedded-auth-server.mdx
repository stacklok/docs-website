...
`scope` value, then later request a wider set of scopes at `/oauth/authorize`.
By default, the embedded authorization server rejects those requests with
`invalid_scope` because the registered client's scope set does not include the
scopes being requested.

To work around this, configure `baselineClientScopes` on the embedded auth
server. The server unions these scopes into every DCR-registered client's scope
set, so clients can request them at `/oauth/authorize` regardless of what they
originally registered with. Each value must already be present in the
upstream-derived `scopesSupported` set; the auth server fails to start if any
value is missing.
