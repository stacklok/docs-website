...
narrower scope set, that acts on behalf of the same user. An agent framework
uses this pattern when it needs to call several downstream services under a
single user's identity without re-prompting the user.
