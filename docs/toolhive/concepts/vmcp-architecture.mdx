...
**Example scenario**: For a PR deployment, merge the PR, wait for tests, ask a
human for approval, deploy only if approved, and notify Slack. vMCP handles this
entire flow declaratively, with automatic rollback on deployment failure.
