...
This means you can combine internal tools with external SaaS MCP endpoints in a
...
docs). Some are container-based MCPServers running in the cluster, while others
are remote SaaS MCP endpoints accessed via MCPRemoteProxy. Instead of
configuring 8 separate connections, they configure one vMCP connection with SSO.
This significantly reduces configuration complexity and makes onboarding new
team members much easier.
