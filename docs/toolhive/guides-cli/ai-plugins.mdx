...
alongside the artifact so consumers can supply it. Keyless-signed plugins need
no such out-of-band step, since the signer identity is verifiable from the
...
Consumers who install an unsigned plugin project-scoped must pass
`--allow-unsigned` on `thv ai-plugin install`.
