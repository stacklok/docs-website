...
## Request requirements for TCP listeners

`thv serve` applies two safeguards to TCP listeners that protect against
cross-origin browser attacks:

- **`Content-Type: application/json` is required** on `POST`, `PUT`, `PATCH`,
  and `DELETE` requests that carry a body. Requests with any other content type
  (or none) receive `415 Unsupported Media Type`. Empty-body mutating routes
  like `stop` and `restart`, and all `GET` and `HEAD` requests, are exempt. The
  `charset` parameter is stripped before comparison, so
  `application/json; charset=UTF-8` matches.
- **`Origin` is validated** when the browser sends the header. On a loopback
  bind (`localhost`, `127.0.0.0/8`, `::1`), the allowlist is derived from the
  bind address; requests whose `Origin` does not match are rejected with `403`.
  On a non-loopback bind, `Origin` validation passes through and a startup
  `WARN` is logged; front the listener with a reverse proxy that enforces
  `Origin`.

Both safeguards apply only to TCP listeners. When you run
`thv serve --socket <PATH>` on macOS, Linux, or Windows, neither check applies.

Most HTTP client libraries send `application/json` automatically. Callers that
don't (for example, `curl -d` and bare `fetch()`) must add the header
explicitly:
