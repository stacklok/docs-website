...
Protocol-scheme builds use a default base image for each language. To override
it, use `--runtime-image`:

```bash
thv build --runtime-image node:20-alpine npx://@modelcontextprotocol/server-filesystem
```
...
```bash
thv build --runtime-add-package git --runtime-add-package ca-certificates \
  uvx://mcp-server-git
```
