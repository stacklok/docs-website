...
The `--secret` flag injects values into a container's environment, so it has no
effect on remote servers. Remote servers authenticate with bearer tokens or
forwarded headers instead. See
[Run remote MCP servers](./run-remote-mcp-servers.mdx#authentication-setup) for
