...
For containerized servers, view the container's logs:
...
All servers (both containerized and remote) run through a proxy process managed
by ToolHive. To view the proxy logs instead of the container logs, use the
`--proxy` flag:
