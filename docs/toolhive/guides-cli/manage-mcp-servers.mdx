...
If you'd rather monitor and operate servers from a single live view than chain
together `thv list`, `thv logs`, and other subcommands, run
[`thv tui`](../reference/cli/thv_tui.md) to launch the interactive terminal
dashboard. It shows a real-time server list, streaming logs, a tool inspector,
and a registry browser. Press `?` for the full key map. The TUI is experimental
and intended as a complement to the CLI.
