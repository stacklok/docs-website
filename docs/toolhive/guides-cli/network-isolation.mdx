...
in the egress proxy by default, even when `insecure_allow_all` is set. Pass the
`--allow-docker-gateway` flag to grant access to `host.docker.internal`,
...
Docker Desktop). The flag alone is enough: you do not need to add the gateway
hostname or IP to `allow_host`.

Gateway access granted by `--allow-docker-gateway` is intentionally
port-independent. It bypasses the permission profile's `allow_port` list so that
host-local services on arbitrary ports (databases on 5432, 3306, 6379,
application servers on 8080, and so on) remain reachable once the flag is
enabled. Reaching the host is already a separate, more-privileged opt-in.
...
To combine gateway access with a restrictive profile for other hosts, pass the
flag alongside your profile. The profile controls non-gateway traffic; the flag
controls gateway access:
...
The MCP server can reach `api.example.com:443` (allowed by the profile) and
`host.docker.internal` on any port (granted by the flag).
...
hostname does not exist. Connect to the Docker bridge gateway IP (typically
`172.17.0.1`) instead; the flag grants access to the bridge IP as well.
