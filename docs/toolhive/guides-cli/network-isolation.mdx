...
`thv run` enables network isolation automatically. Rules are sourced, in this
order, from:
...
- An empty profile that blocks all outbound network access (when neither of the
  above applies)

Passing `--isolate-network` explicitly is equivalent to the default and is still
accepted for clarity in scripts and documentation:
