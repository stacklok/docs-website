...
ToolHive caps the size of inbound HTTP request bodies to the proxy so a large
payload can't exhaust proxy memory before authentication or MCP parsing runs.
The default cap is 8 MiB.

To raise or lower the limit for a specific workload, pass
`--max-request-body-size` on `thv run` (or `thv proxy`) with a value in bytes.
Zero uses the default; negative values are rejected:

```bash
thv run --max-request-body-size 16777216 <SERVER>   # 16 MiB
```

The same limit applies to `thv proxy` when you're fronting an existing HTTP
endpoint:

```bash
thv proxy --max-request-body-size 16777216 <SERVER_NAME>
```

For Kubernetes deployments, set `spec.maxRequestBodySize` on the `MCPServer`
resource. See [MCPServer](../reference/crds/mcpserver.mdx) for the field
reference.
