...
ToolHive lets you install skills from the
[ToolHive Registry](../guides-registry/skills.mdx), OCI registries, or Git
repositories, and manages their lifecycle across multiple AI clients.
...
- The [ToolHive API server](./api-server.mdx) must be running. Start it with:

  ```bash
  thv serve
  ```

- A supported AI client installed (such as Claude Code, OpenCode, or Codex)

## Install a skill

You can install skills from several sources: the ToolHive Registry (by name), an
OCI registry (by reference), or a Git repository (by URL).

### Install from the registry

The simplest way to install a skill is by name. ToolHive looks up the skill in
the configured registry and installs it automatically:
...
```bash
thv skill install ghcr.io/stacklok/skills/toolhive-cli-user:v0.3.0
```
...
```bash
thv skill install git://github.com/example/my-skills@main#my-skill
```
...
Supported clients include `claude-code`, `opencode`, and `codex`.

### Add a skill to a group

You can organize skills into groups, just like MCP servers:
...
If you've created your own skill, you can package it as an OCI artifact and
publish it to a registry for others to install.
