...
Use `--check` to report drift without installing or writing anything - useful as
a CI gate:
...
hash) are reported as not upgradable - there is nothing newer to resolve to.
...
```bash
# Sign with a cosign key on disk (COSIGN_PASSWORD in the `thv serve`
# environment decrypts an encrypted key)
thv skill push ghcr.io/my-org/skills/my-skill:v1.0.0 \
  --key cosign.key
