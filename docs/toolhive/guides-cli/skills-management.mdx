...
When a skill was signed with `--key` rather than keyless, the first
project-scoped install must also pass `--public-key` with the matching cosign
...
ToolHive verifies the signature against the key, then pins the key in the
project's `toolhive.lock.yaml` so subsequent `thv skill sync` and
`thv skill upgrade` runs reuse it automatically. Distribute the public key
alongside the artifact so consumers can supply it. Keyless-signed skills need no
such out-of-band step, since the signer identity is verifiable from the
