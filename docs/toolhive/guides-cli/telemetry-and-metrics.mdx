...
thv run \
  --otel-endpoint api.honeycomb.io \
  --otel-custom-attributes "service.version=1.2.3,api.endpoint=https://api.example.com" \
  fetch
```
...
possible. For example, use `deployment.environment` instead of `env` for better
