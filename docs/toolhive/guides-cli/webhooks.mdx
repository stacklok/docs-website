...
| `url`             | Yes      | HTTPS endpoint to call. HTTP is only allowed when `tls_config.insecure_skip_verify: true`.                                    |
...
- `ignore` — allows the tool call through. Use this for non-critical webhooks
  like logging or enrichment, where availability is not a hard requirement.
...
- [Custom permissions](./custom-permissions.mdx) — configure Cedar-based
  authorization policies for MCP servers
- [Authentication](./auth.mdx) — set up OIDC authentication for MCP servers
