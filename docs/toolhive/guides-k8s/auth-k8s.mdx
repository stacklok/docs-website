...
resolves identity from gjson dot-notation paths into the token response body.
...
must be 1-256 characters long. `identityFromToken` and `userInfo` can both be
set, but only one is used at runtime.
