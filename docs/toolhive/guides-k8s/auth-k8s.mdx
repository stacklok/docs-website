...
this upstream: it derives a non-personally-identifying subject from the access
token (prefixed with `tk-`) and leaves `name` and `email` empty. Use this shape
for OAuth 2.0 servers that don't expose a userinfo endpoint, such as MCP
authorization servers per the
