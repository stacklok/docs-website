...
Check the `MCPOIDCConfig` status to confirm it's valid:

```bash
kubectl get mcpoidc -n toolhive-system
```

The `VALID` column confirms validation passed. To see which workloads reference
this config, query workloads by their `oidcConfigRef`:

```bash
kubectl get mcpservers,mcpremoteproxies,virtualmcpservers -n toolhive-system \
  -o json | jq -r --arg n production-oidc '.items[]
