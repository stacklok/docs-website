...
VirtualMCPServer that references this MCPExternalAuthConfig. It's typically
the external URL that MCP clients use to reach the server.

You still need to register this callback URL with your upstream OAuth2 or OIDC
provider before the flow can complete. Use the same URL on both sides: the
value computed from `resourceUrl` here, and the authorized redirect URI in
