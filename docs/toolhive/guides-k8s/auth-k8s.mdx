...
| `baselineClientScopes` | Optional list of OAuth 2.0 scopes that are unioned into every DCR-registered client's scope set. Use this when MCP clients register with a narrowed `scope` field but later request wider scopes at `/oauth/authorize`. See [Baseline scopes for DCR clients](../concepts/embedded-auth-server.mdx#baseline-scopes-for-dcr-clients). |
