...
- An existing `MCPExternalAuthConfig` resource with `type: embeddedAuthServer`
  ...
  [Set up embedded authorization server authentication](./auth-k8s.mdx))
...
document. Port-forward to the service and query the discovery endpoint:
...
2. The resource has been applied and the operator has reconciled it:
   ```bash
   kubectl describe mcpexternalauthconfig my-embedded-auth-server
   ```
