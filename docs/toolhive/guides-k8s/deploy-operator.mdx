...
`MCPRegistry`, `VirtualMCPServer`, and `EmbeddingServer`. Per-CR
`imagePullSecrets` (for example, `spec.imagePullSecrets` on `MCPRegistry` or
`VirtualMCPServer`, and
`spec.resourceOverrides.proxyDeployment.imagePullSecrets` on `MCPServer` or
`MCPRemoteProxy`) take precedence on name collisions; chart-level entries are
appended additively.

The chart also exposes `operator.imagePullSecrets`, which controls the
operator's _own_ pod only. Use that field if your operator image is in a private
registry, and use `defaultImagePullSecrets` for the workloads it manages.
