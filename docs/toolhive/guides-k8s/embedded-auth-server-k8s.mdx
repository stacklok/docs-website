...
Confidential DCR is registration-driven and unauthenticated. For a client the
operator controls and knows in advance - typically a backend service that
performs RFC 8693 token exchange against the embedded auth server -
pre-provision it with `delegateClients` instead. Each entry references a
...
categorically at admission - the CEL rule cannot express the loopback exception,
so use an `https://` issuer whenever you configure delegate clients.
