...
`trustedIssuers` entry. The example below configures a delegate client named
`coding-agent` to exchange user tokens from Microsoft Entra ID. It uses
`disableUpstreamTokenInjection` because the backend is public: ToolHive
validates the delegated token, then forwards the request without a credential.
