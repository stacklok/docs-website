...
MCPServerEntry is part of an [MCPGroup](../reference/crds/mcpgroup.mdx), which
groups related backend MCP servers together for vMCP discovery. When vMCP starts
in [discovered mode](../guides-vmcp/backend-discovery.mdx), it queries all
...
unique-local addresses, and `*.cluster.local` hostnames. Use this when a Virtual
MCP Server needs to reach a co-located backend inside the cluster so the
backend's own workload-identity authorization still applies.
...
- [MCPServerEntry CRD specification](../reference/crds/mcpserverentry.mdx) -
