...
4. No pods, services, or deployments are created -- the entry is purely a
   catalog record
5. When a VirtualMCPServer in
...
The `transport` field has no default value. Because MCPServerEntry points at
external servers you don't control, ToolHive requires you to specify the
transport explicitly to avoid silent mismatches.

## Configure authentication

When the remote MCP server requires authentication, reference an
[MCPExternalAuthConfig](./auth-k8s.mdx#set-up-embedded-authorization-server-authentication)
resource to configure token exchange. The MCPExternalAuthConfig must exist in
the same namespace as the MCPServerEntry.
...
```yaml {9-12} title="tls-entry.yaml"
apiVersion: toolhive.stacklok.dev/v1alpha1
kind: MCPServerEntry
metadata:
  name: internal-tool
...
```yaml {9-11} title="header-entry.yaml"
apiVersion: toolhive.stacklok.dev/v1alpha1
kind: MCPServerEntry
metadata:
  name: my-remote-tool
...
This example creates all the resources needed for an MCPServerEntry with
...
- [Proxy remote MCP servers](./remote-mcp-proxy.mdx) when you need full proxy
  capabilities like OIDC validation and Cedar policies
