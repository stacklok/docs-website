...
2. **Apply updated CRDs** for the target version (see below).
...
### Applying updated CRDs

Use whichever method matches your initial installation. See
[Deploy the operator](./deploy-operator.mdx#install-the-crds) for full details.
...
Replace `<VERSION>` with the target release tag (e.g., `v0.17.0`). See
[Deploy the operator](./deploy-operator.mdx#install-the-crds) for the full list
of individual CRD URLs if you only install a subset.
