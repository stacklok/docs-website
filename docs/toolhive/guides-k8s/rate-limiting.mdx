...
resource that owns the bucket. Prometheus appends `_total` to counter names and
`_seconds` to the latency histogram (with the standard `_bucket`, `_sum`, and
`_count` series suffixes).
...
`toolhive_rate_limit_redis_errors` alongside decisions to catch periods where
limits stopped applying.
...
- [Telemetry and metrics](./telemetry-and-metrics.mdx) to enable OpenTelemetry
  metrics collection for the metrics above
