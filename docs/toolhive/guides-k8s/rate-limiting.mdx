...
:::note[Error code change in v0.41.0]

The JSON-RPC `error.code` for rate limiting moved from `-32029` to `429` in
ToolHive v0.41.0 because the MCP 2026-07-28 spec reserves the `-32020` to
`-32099` JSON-RPC code range for spec use. The HTTP `429` status, the
`Retry-After` header, and the `data.retryAfterSeconds` field are unchanged;
branch on those values for your backoff logic.
