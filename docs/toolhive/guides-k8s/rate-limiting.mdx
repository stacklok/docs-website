...
ToolHive v0.41.0 to avoid a range reserved by the MCP 2026-07-28 spec. The HTTP
`429` status, the `Retry-After` header, and the `data.retryAfterSeconds` field
are unchanged; branch on those values for your backoff logic.
