...
- Familiarity with the
  [embedded authorization server](./embedded-auth-server-k8s.mdx) setup
...
:::tip[Cluster-wide default]

If most workloads in your cluster use the same Redis for horizontal scaling, set
`operator.defaultRedis` on the Helm values for the ToolHive operator instead of
configuring `sessionStorage` on every CR. See
[Set a cluster-wide default Redis](./deploy-operator.mdx#set-a-cluster-wide-default-redis).
...
- [Kubernetes CRD reference: SessionStorageConfig](../reference/crds/mcpserver.mdx)
- [Kubernetes CRD reference: auth server storage](../reference/crds/virtualmcpserver.mdx)
