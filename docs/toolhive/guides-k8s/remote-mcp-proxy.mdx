...
:::info[OIDC CA bundle is mounted into the proxy pod]

When you set `caBundleRef` on an MCPOIDCConfig referenced by an MCPRemoteProxy,
the operator mounts the referenced ConfigMap into the proxy pod and uses it for
the proxy's OIDC HTTPS calls (issuer discovery and JWKS fetch). A
`CABundleRefValidated` condition reflects whether the bundle was resolved
successfully; if the ConfigMap is missing or the key contains no valid PEM
certificates, reconciliation fails before the pod starts rather than the pod
crashing later with an opaque TLS error.
