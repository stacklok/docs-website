...
The proxy caps inbound request bodies at 8 MiB by default so a large payload
can't exhaust proxy memory before authentication or MCP parsing runs. Set
`spec.maxRequestBodySize` to override the cap for a specific workload. The value
is in bytes; zero uses the default:

```yaml title="my-mcpserver-body-limit.yaml"
apiVersion: toolhive.stacklok.dev/v1beta1
kind: MCPServer
metadata:
  name: fetch
...
spec:
  ...
  mcpPort: 8080
  proxyPort: 8080
  maxRequestBodySize: 16777216 # 16 MiB
```
...
- `proxyDeployment` - overrides for the proxy Deployment. Supports `labels` and
  ...
  (environment variables for the proxy container), `imagePullSecrets`, and the
  scheduling fields `nodeSelector`, `tolerations`, and `affinity`.
...
- **Scheduling** with `proxyDeployment.nodeSelector`,
  `proxyDeployment.tolerations`, and `proxyDeployment.affinity` lands the proxy
  Deployment on a pre-warmed or dedicated node pool. Use these when the proxy
  should follow specific hardware or licensing constraints:
  ...
  If `podTemplateSpec` also sets scheduling fields,
  `podTemplateSpec.nodeSelector`/`affinity` sub-fields win on any keys they
  specify and `podTemplateSpec.tolerations` replaces the list from
  `resourceOverrides.proxyDeployment` outright.
