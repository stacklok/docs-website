...
requests traced, as a value between 0 and 1.0. The default is `0.05` (5%).
...
A minimal collector configuration that receives OTLP data and exports traces and
metrics:
...
your base64-encoded `instanceID:apiToken` credentials in a Kubernetes Secret
referenced by `sensitiveHeaders`.
