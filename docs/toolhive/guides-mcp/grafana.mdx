...
`http://host.docker.internal:3000` (rather than `localhost`, which points to the
server's own container) and enable **Allow host machine access** on the
**Network Isolation** tab so the server can reach it. On Docker and Podman
Desktop (macOS and Windows), `host.docker.internal` resolves to the host; on
Linux, use the Docker bridge gateway IP instead (typically `172.17.0.1`).
...
On the **Network Isolation** tab, **Allowed destinations** defaults to
**Anywhere**. Change it to **Selected destinations** to restrict the server's
outbound access to your Grafana instance's host and ports.
...
Here, `http://host.docker.internal:3000` reaches a Grafana instance on your host
machine, since `localhost` would resolve to the server's own container. The
`--allow-docker-gateway` flag is required because ToolHive's network isolation
blocks Docker gateway addresses by default. On Docker and Podman Desktop (macOS
and Windows), `host.docker.internal` resolves to the host; on Linux, use the
Docker bridge gateway IP (typically `172.17.0.1`), which the flag also allows.
See [network isolation](../guides-cli/network-isolation.mdx) for details.
...
- For read-only use cases like querying metrics, viewing dashboards, or triaging
  incidents, pass the `--disable-write` argument to block mutating tools (such
  as creating incidents, updating dashboards, and managing alert rules) while
  keeping all read operations available. The MCP server enforces this as defense
  in depth, so pair it with a read-only service account token rather than
  relying on it alone.
