...
`<issuer>/oauth/callback`, as shown above. Don't rely on the default computed
from `resourceUrl`: since `resourceUrl` includes the `/mcp` path here, the
default would land on the same unserved path.
