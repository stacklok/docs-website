...
:::note[Known issue: `level` field may show as `INFO+2`]

In some versions of the Registry Server, the `level` field is serialized as
`"INFO+2"` rather than `"AUDIT"` (tracked in
[#826](https://github.com/stacklok/toolhive-registry-server/issues/826)). If
