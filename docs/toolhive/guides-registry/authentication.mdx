...
If logs show `failed to fetch JWKS`, `connection refused`, or
`x509: certificate signed by unknown authority` when fetching
`${issuerUrl}/.well-known/openid-configuration`:

1. **DNS and network egress.** Confirm the Registry Server pod or host can
   resolve and reach the issuer hostname. Test with
   `curl ${issuerUrl}/.well-known/openid-configuration` from the same network
   context.
