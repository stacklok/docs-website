...
3. **Subsequent versions must have the same claims as the first.** Once you
   ...
   versions must carry the exact same claims (including the case where the first
   version had no claims). This prevents accidentally narrowing or broadening
   visibility across versions.
...
- `GET /v1/me` reports every defined role for authenticated callers, reflecting
  the effective access they have when authorization is not configured
