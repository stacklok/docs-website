...
| `manageEntries`    | Publish, delete, and manage claims on MCP server versions, skills, and plugins |
...
entries** during sync. Every MCP server, skill, or plugin ingested from that
source carries the source's claims.
...
`type` path parameter is `server`, `skill`, or `plugin`, and the endpoints
require the `manageEntries` role. Because entry names contain a slash separating
namespace from name, URL-encode the slash as `%2F` so the path is treated as a
single `{name}` path parameter.
...
- **List source/registry entries**: `GET /v1/sources/{name}/entries` and
  `GET /v1/registries/{name}/entries` return the raw entries (servers, skills,
  and plugins with versions and claims) in a source or registry. These endpoints
  follow the same claim scoping: the parent source or registry must be
