...
- A **registry** is a named API surface that aggregates one or more sources.
  ...
  `/registry/{registryName}/v0.1/servers`). A registry can also have claims that
  act as an access gate: only callers whose JWT claims satisfy the registry's
  claims can access its entries.
...
API-managed source for directly publishing and deleting MCP servers, skills, and
plugins via the API. Does not sync from external sources.
...
- `POST /v1/entries` - Publish new server, skill, or plugin versions
...
- `PUT /v1/entries/{type}/{name}/claims` - Update authorization claims on a
  published entry
- `GET /registry/{registryName}/v0.1/...` - List and retrieve servers, skills,
  and plugins
- [Manage skills](./skills.mdx) and [Manage plugins](./plugins.mdx) via the
  extensions API
...
Each registry entry has a version that appears in its identity and in
...
stored as distinct rows. The value must be 255 characters or fewer.
...
identity and URLs, rather than being normalized to `2.1.0`.
...
move to that tag on the next reconcile, and the previous `1.0.0` version row is
...
basis. Only applicable to synced sources (Git, API, File). Filters apply to MCP
servers, skills, and plugins. Skills and plugins are matched using
`namespace/name` against the name patterns.
...
The server can emit structured JSON audit events for every admin and consumer
API operation. Audit logging is disabled by default. See the
[Audit logging](./audit-logging.mdx) guide for configuration options, event
types, and output destination setup.
