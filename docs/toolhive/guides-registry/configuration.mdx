...
- `POST /v1/entries` - Publish new server or skill versions (requires a `claims`
  object when authentication is enabled; see
...
- `PUT /v1/entries/{type}/{name}/claims` - Update authorization claims on a
  published entry
- Consumer reads at `/registry/{registryName}/v0.1/...` for listing and
  retrieving servers and skills
