...
Pick one of the following mechanisms to provide database credentials.
...
When multiple static sources are configured, the server resolves them in this
order of precedence:
...
2. **Config field or environment variable** - the `password` and
   ...
   `THV_REGISTRY_DATABASE_MIGRATIONPASSWORD` environment variables (environment
   values override values set in YAML)
...
[deploy guides](./deploy-operator.mdx) for complete examples.
