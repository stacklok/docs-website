...
If you install the chart as a subchart in an umbrella that also runs other
PostgreSQL-backed services, set the database connection once at the umbrella
level under `global.postgres` instead of repeating it per subchart:
...
working. Only `host`, `port`, and `sslMode` are inherited - `user`, `database`,
