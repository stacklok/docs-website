...
If you install the chart as a subchart in an umbrella chart that also runs other
PostgreSQL-backed services, set the database host once at the umbrella level
under `global.postgres` instead of repeating it per subchart:
