...
catalog of MCP servers, skills, and plugins for your organization. Use it when
you want a trusted catalog for discoverability, team-scoped access, publishing,
or multi-source aggregation.
...
The [Enterprise Cloud UI](../../platform/enterprise-cloud-ui/index.mdx) provides
a browser-based management console for the Registry Server. The open source
ToolHive Cloud UI [is no longer supported](../guides-cloud-ui/index.mdx).

:::note[Registry Server vs. built-in registry]

This section covers the **Registry Server** for hosting and curating your own
MCP server, skills, and plugins catalog.
...
- Publish MCP servers, [skills](../concepts/skills.mdx), or plugins for internal
  use
...
- A **source** is where data comes from: a Git repository, an upstream API, a
  ...
  or receives MCP servers, skills, and plugins.
- An **entry** is an individual MCP server, skill, or plugin stored in the
  database. Sources produce entries; each entry belongs to exactly one source.
...
The Registry Server aggregates MCP server, skill, and plugin metadata from
various sources and exposes it through a standardized API. When you start the
server, it:
...
- **Plugins registry**: Publish and discover plugin bundles (slash commands,
  subagents, Agent Skills, hooks, MCP server configs, LSP servers) via the
  extensions API and synced sources
...
1. **Managed** - A fully-managed MCP source
   ...
   - Supports publishing MCP servers, skills, and plugins via the `/v1/entries`
     admin API

2. **Upstream API** - Sync from upstream MCP Registry APIs
   - Supports federation and aggregation scenarios
   - Syncs MCP servers, skills, and plugins from upstream sources
...
4. **Git** - Clone and sync from Git repositories
   ...
   - Syncs MCP servers, skills, and plugins
   - Does not support publishing
...
- [Follow the Registry Server quickstart](./quickstart.mdx) to deploy a local
  environment and query its API
- [Choose a deployment method](./deployment.mdx) for a production environment
