...
entries from the same sources as MCP servers and skills. A **plugin** is the
bundle unit pioneered by Claude Code: a directory declared by a
`.claude-plugin/plugin.json` manifest that groups slash commands, subagents,
Agent Skills, hooks, MCP server configs, and LSP servers into a single
distributable artifact. This guide covers the full lifecycle: publishing,
listing, retrieving, and deleting plugins.
