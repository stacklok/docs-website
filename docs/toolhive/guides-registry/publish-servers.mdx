...
Your JWT must satisfy the managed source's claims, and the publish claims must
be a subset of your JWT claims. Subsequent versions of the same server must
carry the same claims as the first version. See
