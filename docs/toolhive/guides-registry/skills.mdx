...
The example below shows the request shape without claims. When authentication is
enabled, the body must also include a `claims` object at the top level or the
server returns `400 Bad Request`. See [Claims](#claims) below.
