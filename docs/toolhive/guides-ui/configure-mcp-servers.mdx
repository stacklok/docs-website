...
Choose how the MCP server reaches the network. The **Network access** tab lets
you pick between no isolation, host networking, or routing outbound HTTP and
HTTPS traffic through ToolHive's egress proxy so it can only reach the
destinations you allow. See the [Network isolation](./network-isolation.mdx)
guide for details.
