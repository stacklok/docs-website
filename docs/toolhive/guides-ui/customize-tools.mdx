...
Tools that the registry expects but the running server doesn't expose are listed
under `Missing from server:`, with one badge per tool. This typically means the
upstream MCP server changed its tool surface without a matching registry update,
or that the running container doesn't match the image referenced by the registry
entry.
...
- Try out your customized tools in the [Playground](./playground.mdx)
