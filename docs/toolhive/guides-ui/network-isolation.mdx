...
Registry servers default to **Isolate behind an HTTP proxy** unless the registry
entry declares a different mode. Custom servers default to **No isolation**;
switch to the proxy mode to opt in.

### Restricting proxied traffic

When you choose **Isolate behind an HTTP proxy**, an **Allowed destinations**
radio appears:
...
If **Selected destinations** is chosen, any safe defaults defined by the
...
purposes; the form displays a warning when this configuration is detected.
...
To allow an MCP server to access other workloads on the same network, choose
**Isolate behind an HTTP proxy** with **Selected destinations**, then list the
appropriate hostnames and ports. This is commonly needed when your MCP server
needs to communicate with databases, APIs, or other services that are running on
your local host during development.
...
1. Check the network access mode and allowed destinations. Open the server's
   ...
   server needs are listed. Proxied network isolation only supports HTTP and
   HTTPS, so direct TCP connections (databases, custom protocols) won't work in
   this mode.
