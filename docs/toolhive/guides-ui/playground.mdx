...
The Stacklok Gateway option in **Provider Settings** lets you chat with GPT,
Claude, and Gemini models through an OIDC-protected LLM gateway rather than
configuring an API key per provider. ToolHive routes requests to a loopback
proxy that injects a fresh JWT on each call, so no API key is stored in Studio.
Under the hood, ToolHive uses the same connection settings as
[`thv llm setup`](../reference/cli/thv_llm_setup.md) and manages the local proxy
started by [`thv llm proxy start`](../reference/cli/thv_llm_proxy_start.md).
...
4. If the panel shows **Sign-in required**, click **Sign in** to complete the
   OIDC flow in your browser. When it returns **Signed in**, the gateway is
   ready to use.
...
and click the trash icon, then **Save**. Studio stops the proxy it started and
removes the gateway models from the picker. The `thv llm config` on disk stays
intact so CLI clients that use the same gateway continue to work.
