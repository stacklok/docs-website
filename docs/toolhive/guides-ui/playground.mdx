...
3. **Select MCP tools**: Click the tools icon in the chat toolbar to manage
   ...
   Playground tool selection applies only to the active thread.
...
- **Copy**: copies the message text to your clipboard. Tool inputs, tool
  outputs, and internal reasoning are excluded; tool result blocks have their
  own per-block **Copy** button.
