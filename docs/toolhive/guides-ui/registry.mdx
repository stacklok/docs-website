...
When you've configured a custom registry, a **Refresh** button appears in the
page header. Click it to pull the latest registry metadata and server list on
demand, without having to restart ToolHive or reopen the page. The button is
hidden for the built-in registry.
