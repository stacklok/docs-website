...
The default ToolHive registry contains only individual servers. To install
groups from the registry, you need to configure a
[custom registry](../tutorials/custom-registry.mdx) that defines them.

### Browse groups

When using a custom registry that includes groups, they appear in the registry
grid alongside individual servers. Groups allow you to install multiple related
MCP servers together as a single unit.

### View group details

Click on a group to see detailed information:

- **Group name and description**: Explains the purpose of the group
- **Server list**: A table showing all servers in the group with their
  descriptions
- **Install group button**: Starts the group installation wizard

### Install a group

To install a group:

1. Click the **Install group** button to open the installation wizard
2. The wizard guides you through configuring each server in the group one at a
   time. Configuring each server works the same way as
   [installing an individual server](./run-mcp-servers.mdx#configure-the-server).
3. Each form shows your progress (for example, "Installing server 1 of 3")
4. Some servers may require custom configuration such as environment variables
   or secrets, while others work with defaults
5. Click **Next** to proceed to the next server, or **Finish** on the last
   server
6. Once complete, all servers are installed and grouped together

After installation, the group appears in the **Groups** section where you can
manage all the servers together.
