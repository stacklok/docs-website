...
description:
  Install and run MCP servers from the registry or custom images using the
...
{/* prettier-ignore */}
<Heading as='h3' id='from-a-docker-image'>From a Docker image</Heading>
...
{/* prettier-ignore */}
<Heading as='h3' id='from-a-source-package'>From a source package</Heading>
...
For MCP servers installed locally from the registry, ToolHive surfaces upgrade
controls on the server card when a newer image tag is available. The upgrade
controls only appear when all of the following are true:

- The server runs locally, not as a remote URL install.
- The server's registry entry includes a Docker image.
- The local image tag differs from the tag in the registry.

When an upgrade is available, ToolHive offers two entry points to start the
upgrade:

- On the server card, an amber circular up-arrow icon button appears alongside
  the **start** and **stop** controls. Hover over it to see a tooltip
  `Update available: <LOCAL_TAG> → <REGISTRY_TAG>`, where `<LOCAL_TAG>` is the
  currently installed tag and `<REGISTRY_TAG>` is the new registry tag.
- The server's overflow (⋮) menu also includes an **Update to `<TAG>`** item.
  This item is disabled while the workload is being deleted, removed, or
  updated.

Selecting either entry point opens the **Update to latest version** dialog.
...
- **No configuration drift**: The dialog shows
  `No configuration changes detected between versions.` Click **Update** to pull
  the new image and restart the server with your existing configuration, or
  **Cancel** to dismiss the dialog.

- **Configuration drift**: The dialog lists added variables under
  `New variables in this version:` with `secret`, `env`, and `required` badges
  that indicate the variable type and whether a value is required. Variables
  that the registry removed but you previously set appear under
  `Variables no longer in registry:`. Click **Edit and review** to open the edit
  configuration flow with the new image tag pre-applied and the new variables
  added to the form, where you can supply values, adjust secrets, and remove
  obsolete variables before saving. Click **Cancel** to keep the current
  version.
