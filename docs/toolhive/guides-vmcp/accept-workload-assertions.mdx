...
`authServerConfig`. For the MCPServer field reference, see
...
pre-register. To accept the assertion in the first place, ToolHive needs an
`inboundGrants.jwtBearer.issuerPolicies` entry naming the workload's subject and
the resource it may request. What that entry looks like depends on the identity
provider:
...
`VirtualMCPServer` needs a placeholder `upstreamProviders` entry pointing at any
reachable OAuth2 endpoint, with credentials that are never actually used:
