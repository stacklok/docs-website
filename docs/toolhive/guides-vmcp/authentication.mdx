...
spec:
  ...
  xaa:
    ...
    # Effectively required when the VirtualMCPServer has more than one
    # upstream provider: the controller auto-populates this with the first
    # configured provider, which may be the wrong IdP. Must match the name
    # of an entry under authServerConfig.upstreamProviders.
