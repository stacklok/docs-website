...
among others), set `baselineClientScopes` so the embedded auth server unions
those scopes into every registered client. Each value must be present in the
upstream-derived `scopesSupported` set. See
