...
For conceptual background on the embedded authorization server including token
storage and forwarding, see
[Backend authentication concepts](../concepts/backend-auth.mdx#embedded-authorization-server).
For configuring the embedded auth server on individual MCPServer resources
(single upstream provider), see
