...
The `xaa` outgoing auth strategy handles the cross-domain case, where a backend
trusts a different authorization server than the one that issued the client's
token, and no federation is configured between them. XAA performs a two-step
...
- **Step A ([RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693))**: vMCP
  ...
  scoped to the target authorization server.
...
Use `xaa` when [`tokenExchange`](#token-exchange-with-upstream-tokens) doesn't
apply (the backend doesn't trust the client's IdP) and
[`upstreamInject`](#upstream-token-injection) doesn't apply (the user has not
previously authenticated to the backend's provider). XAA requires the
[embedded authorization server](#embedded-authorization-server) so the user's
upstream ID token is available at request time.

Create an `MCPExternalAuthConfig` with `type: xaa`. The IdP and target
authorization server credentials are stored in Kubernetes Secrets:
...
spec:
  ...
  xaa:
    ...
    # Required: identifies the target resource AS for the ID-JAG audience claim
    ...
    # Optional: pin to a specific upstream provider. When empty, the
    # controller populates it with the first configured upstream provider.
    subjectProviderName: my-idp
```

Reference it from the VirtualMCPServer's outgoing auth:
...
**Required fields**: `idpTokenUrl`, `targetTokenUrl`, and `targetAudience`. Both
token URLs must be HTTPS. Set `insecureTargetTokenUrl: true` to permit plain
HTTP on `targetTokenUrl` for in-cluster or development endpoints only, never in
production.
