...
- `healthy`: Backend is responding correctly with good performance
- `degraded`: Backend is operational but experiencing issues (slow responses >5s
  or recovering from failures)
- `unhealthy`: Backend not responding or health checks failing
- `unauthenticated`: Backend authentication failed
- `unknown`: Health check not yet performed

:::info[Status representations]

The `/status` endpoint uses internal health values (`healthy`, `degraded`,
`unhealthy`, `unauthenticated`, `unknown`).

The VirtualMCPServer CRD `status.discoveredBackends[].status` field uses
user-facing values (`ready`, `degraded`, `unavailable`, `unknown`) where `ready`
= healthy, `unavailable` = unhealthy or unauthenticated.

Both representations show the same underlying health state with different
terminology.
...
consistently failing backends. When a backend exceeds the failure threshold, the
circuit breaker opens and stops sending requests for a cooldown period.
    ...
    operational:
      failureHandling:
        circuitBreaker:
          ...
          # How long to wait before attempting recovery (default: 60s)
          timeout: 60s
```
...
- **Open**: Too many failures, requests blocked for timeout duration
...
Check backend health via the VirtualMCPServer status:

```bash
kubectl get virtualmcpserver my-vmcp -n toolhive-system -o json | jq '.status.discoveredBackends'
```

Returns an array of backends with their health status:
...
[
  {
    ...
    "authType": "token_exchange",
    "lastHealthCheck": "2025-02-09T10:30:00Z",
    "message": "Healthy",
    "circuitBreakerState": "closed",
    "circuitLastChanged": "2025-02-09T10:00:00Z",
...
**Status values** (CRD representation):

- `ready`: Backend is healthy and accepting requests with good performance
- `degraded`: Backend operational but with issues (slow responses >5s or
  recovering from failures)
- `unavailable`: Backend not responding, health checks failing, or
...
`unavailable`, `unknown`). The `/status` HTTP endpoint uses internal health
values (`healthy`, `degraded`, `unhealthy`, `unauthenticated`, `unknown`). These
represent the same health state with different terminology.
