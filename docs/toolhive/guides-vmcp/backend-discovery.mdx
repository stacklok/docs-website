...
- `mcpservers`, `mcpremoteproxies`, `mcpserverentries`: Discover backend
  workloads
...
[MCPExternalAuthConfig](../reference/crds/mcpexternalauthconfig.mdx) resources,
not inline configuration. Create the MCPExternalAuthConfig resource first, then
reference it by name. See the [Authentication guide](./authentication.mdx) for
complete examples.
...
| `unauthenticated` | Misconfiguration: backend returned 401/403 but no outgoing auth strategy is configured on the backend target | `unauthenticated` |
| `unknown`         | Health check not yet performed (initial state)                                                               | `unknown`         |

:::note[Auth challenges count as healthy]

Health probes deliberately run with no user context, so a 401/403 from a backend
that **does** have an outgoing auth strategy configured (for example
`upstream_inject` or `token_exchange`) is reported as `healthy`. The challenge
proves reachability and a working auth layer; per-request user authentication
happens later when an authenticated user session drives discovery.

`unauthenticated` is reserved for the misconfiguration case where the backend
demands authentication but no outgoing auth strategy was wired up. Backends in
this state are excluded from capability aggregation and do **not** count toward
the `/status` `healthy: true` aggregate.
...
`unavailable`, `unauthenticated`, `unknown`) as shown in the "CRD Status" column
above.
...
4. Optionally reduce RBAC permissions by removing read access to MCPServer,
   MCPRemoteProxy, and MCPServerEntry resources (keep status update permissions)

### From inline to discovered

1. Ensure backend MCPServer, MCPRemoteProxy, and/or MCPServerEntry resources
   exist in the group
...
2. **Backend resources not referencing the correct group**
   ...
   ```bash
   kubectl get mcpserver,mcpremoteproxy,mcpserverentry -n toolhive-system \
...
2. **Check role permissions**
   ...
   - `mcpgroups`, `mcpservers`, `mcpremoteproxies`, `mcpserverentries`: get,
     list, watch
...
spec:
  config:
    backends: [] # invalid: an empty array fails validation
```
...
spec:
  config:
    backends: [...] # invalid: remove this when source is discovered
```

</details>

<details>
<summary>Health check failures or backends marked degraded/unhealthy</summary>

If backends bounce between healthy and unhealthy, or sit in `degraded`, the
issue is usually slow backends, network policies, or missing auth config. See
[Failure handling troubleshooting](./failure-handling.mdx#troubleshooting) for
tuning `healthCheckTimeout`, `unhealthyThreshold`, and circuit breaker recovery.
