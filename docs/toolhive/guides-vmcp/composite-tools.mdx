...
A composite tool can carry the same MCP tool annotations (`readOnlyHint`,
`destructiveHint`, `idempotentHint`, `openWorldHint`, `title`) that individual
backend tools do. Clients that key off these hints, for example to prompt before
running a destructive tool, use them to decide how to present the composite
tool.
...
Most backend tools declare no annotations today, so composite tools with a
derived floor typically advertise `destructiveHint: true` and
`openWorldHint: true`. This matches the MCP specification's defaults for absent
annotations, but clients that key off explicit hints may begin prompting for
confirmation on composite tools that previously carried none. Declare
`annotations` explicitly to override the floor when the derived values are
stricter than the workflow warrants.
