...
Wire it into a `VirtualMCPServer` with `spec.config.compositeToolRefs`:
...
- **JSON text backends**: If a backend returns data as JSON text and downstream
  ...
  the JSON string. Go template rendering converts `null` to an empty string, and
  an empty string cannot be coerced to `integer`, `number`, or `boolean` output
  property types.
...
An empty string (which is what you get when a template expression resolves to
