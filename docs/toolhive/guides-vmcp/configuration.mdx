...
MCPRemoteProxy resources proxy external remote MCP servers, allowing you to
include SaaS MCP endpoints in your vMCP aggregation:
...
spec:
  ...
  # Validate incoming requests using Kubernetes service accounts
  oidcConfig:
    type: kubernetes
```

The `oidcConfig` field configures how the proxy validates incoming requests. When
using `type: kubernetes`, the proxy trusts requests authenticated via Kubernetes
service accounts, which is suitable for in-cluster communication with vMCP.

If the upstream remote server requires authentication, use `externalAuthConfigRef`
to configure credentials. See
[Proxy remote MCP servers](../guides-k8s/remote-mcp-proxy.mdx#authenticate-to-upstream-remote-servers)
for examples.
