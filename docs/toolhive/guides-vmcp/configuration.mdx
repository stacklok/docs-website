...
default. Values below 30s that were previously silently ignored will now cut
slow requests. If you had configured a short value as aspiration, either raise
it back to `30s` to preserve the earlier behavior or verify your slowest
`tools/call` completes within the window. Values above 30s now hold an in-flight
request handler and an upstream connection for the full duration; size replica
