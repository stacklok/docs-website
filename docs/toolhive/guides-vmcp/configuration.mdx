...
As of v0.20.0, the `groupRef` field on all CRD types changed from a bare string
to a typed struct with a `name` sub-field. For VirtualMCPServer,
`spec.config.groupRef` is deprecated in favor of the top-level `spec.groupRef`.
The [CRD reference](../reference/crd-spec.md) will be updated with the v0.20.0
operator release.
