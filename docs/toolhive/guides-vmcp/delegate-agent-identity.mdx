...
`azp` when you leave it unset. `allowedDelegateClients` scopes delegation to
specific ToolHive clients; set it to `["*"]` to permit any confidential client
holding the token-exchange grant type instead of naming clients individually.
The wildcard must stand alone, and combining it with specific client IDs is
