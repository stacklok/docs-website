...
ticket, calling a tool through a Virtual MCP Server (vMCP)), you usually want
ToolHive to record both facts: who the human is, and which agent actually made
the call, so audit logs and authorization policies can tell "Alice, directly"
apart from "an agent, acting for Alice." RFC 8693 token-exchange delegation is
how ToolHive does this: a pre-provisioned client exchanges a user's token for
one that names both the user and the agent acting for them.
...
If you're deciding which of ToolHive's ways to get a token fits your case, see
the comparison in
