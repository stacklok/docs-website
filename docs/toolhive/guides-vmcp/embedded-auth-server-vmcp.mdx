...
Set `allowMayAct: true` to treat this issuer's `may_act` claim as a delegation
signal. It defaults to `false` because `may_act` bypasses `allowedActors`, so
you must opt in each external issuer explicitly. Enabling it is rejected when
`allowedDelegateClients` is `["*"]`.

For the full field list (including `actorClaim`, `jwksUrl`, `allowPrivateIPs`,
and `insecureAllowHTTP`) and how the delegated token's `sub` and `act` claims
are constructed, see
