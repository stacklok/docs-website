...
By default, DCR only issues public clients. Set
`allowConfidentialClientRegistration: true` to accept `client_secret_basic` and
`client_secret_post` registrations and mint a real `client_secret` (returned
exactly once):
...
`/oauth/register` is unauthenticated, so this lets any caller who can reach the
endpoint mint a client credential. Combining this with `insecureAllowHTTP: true`
is rejected at admission; combining it with a plain-HTTP loopback issuer
requires `insecureAllowConfidentialOverLoopbackHTTP: true` to opt in.

For clients that register as public (`token_endpoint_auth_method: "none"`) but
then refuse to proceed without a `client_secret`,
`forceConfidentialRedirectUris` issues a secret when the registration's
`redirect_uris` contains an exact match for a listed URI. Every entry must be an
`https` non-loopback URI:
...
For the full validation rules and the background on why these flags exist, see
...
against the vMCP embedded auth server (typically a backend service the operator
controls), add a `delegateClients` entry:
