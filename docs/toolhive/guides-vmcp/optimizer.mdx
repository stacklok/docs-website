...
managed TEI path ignores it). Header names must be valid RFC 7230 tokens.
`Authorization` and `Content-Type` are reserved: the client sets `Authorization`
from `OPENAI_API_KEY` and sends `Content-Type` itself, and neither can be
overridden through `embeddingHeaders`.
