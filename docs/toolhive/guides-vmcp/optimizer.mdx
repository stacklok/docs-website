...
an external service that speaks the OpenAI `/embeddings` API - OpenAI itself,
Azure OpenAI, or another OpenAI-compatible gateway. Use this when you already
operate a centralized embedding service and don't want a second copy running per
vMCP, or when you need a hosted model.
