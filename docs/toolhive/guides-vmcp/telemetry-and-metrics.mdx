...
- **Pull via Prometheus**: Set `spec.prometheus.enabled: true` to expose a
  `/metrics` endpoint on the dedicated diagnostics port (`9464` by default,
  overridable via `spec.config.telemetry.prometheusPort` on the
  `VirtualMCPServer`). By default, the endpoint is also served on the vMCP
  transport port for compatibility; set `metricsOnTransportPort: false` to
...
When [rate limiting](../guides-k8s/rate-limiting.mdx) is enabled on the
...
For the counting semantics (allowed increments per bucket, rejected only for the
first rejected bucket) and example PromQL queries, see
