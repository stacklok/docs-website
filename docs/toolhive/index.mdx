...
hide_title: true
description:
  ToolHive helps you run, govern, and connect MCP servers and agent skills
  across the desktop app, CLI, Kubernetes Operator, vMCP, and Registry Server.
---
...
and managing Model Context Protocol (MCP) servers and agent skills. It is the
MCP and skills management capability of Stacklok Enterprise, available as the
open source core you can also run on its own.
...
identity provider integrations (Okta, Entra ID), hardened images, an
[Enterprise Cloud UI](../platform/enterprise-cloud-ui/index.mdx) for web-based
catalog management, and platform capabilities for teams running MCP in
production.
...
- [**ToolHive CLI**](./guides-cli/index.mdx): A command line interface to deploy
  ...
  agent skills, AI-tool plugins, telemetry, and fine-grained authorization
  policies.
...
The ToolHive UI and ToolHive CLI are how users discover, configure, and run MCP
servers. Choose the ToolHive UI app for a cross-platform desktop experience, or
the ToolHive CLI for terminal workflows and automation.
...
1. **Admins** curate and organize MCP servers in the **Registry**, configuring
   access and policies.
2. **Users** discover, configure, and run MCP servers through **ToolHive UI** or
   the **ToolHive CLI**, and ToolHive orchestrates deployment and access.
...
- Use [ToolHive UI](https://github.com/stacklok/toolhive-studio) for a
  cross-platform desktop experience
- Use the [ToolHive CLI](./guides-cli/quickstart.mdx) for terminal workflows and
  automation
- Discover, configure, and run MCP servers
- Automatically configure connected AI clients to use managed MCP servers
