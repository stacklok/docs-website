...
[Configure secure ingress for MCP servers on Kubernetes](./ingress-ngrok.mdx).
...
permissions. Authorization is now at the service level: there are no `aws-mcp:*`
actions. Verify the permission policy attached to the role includes the actions
the MCP tool needs:
