...
role-based access control (RBAC) policies. Writers get full access while readers
see only read-only tools.

:::note[Aggregating multiple backends behind vMCP?]

This tutorial secures a single MCPServer. To put multiple backends behind one
endpoint with the Virtual MCP Server (vMCP) embedded auth server, see
[Connect ToolHive to Okta](./vmcp-okta.mdx) instead.
