...
`subjectClaim: oid` pins the user's identity to the immutable Object ID claim
rather than the default `sub`, which Entra rotates per application; without
this, the same user signing in to a different app appears as a different
principal. The `profile` scope is required for Entra to emit `oid` in the ID
token.
