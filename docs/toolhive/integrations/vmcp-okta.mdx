...
- [ ] Issuer URL: `https://<OKTA_DOMAIN>/oauth2/<AUTH_SERVER_ID>`
...
- Audience: `https://<VMCP_ENDPOINT>` (this becomes the `aud` claim)
- Note the **Issuer URI**
...
- Sign-in redirect URI: `https://<VMCP_ENDPOINT>/oauth/callback`
...
restarts. See
[Configure the embedded auth server](../guides-vmcp/embedded-auth-server-vmcp.mdx#configure-the-embedded-auth-server).
...
(see the filter type warning in step 4).
