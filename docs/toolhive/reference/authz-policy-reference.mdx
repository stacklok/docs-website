...
vMCP's default conflict-resolution prefixes each tool with its backend ID (for
example `resource.name like "my-backend-name_*"`) — or configure the policy on
that backend's own `MCPAuthzConfig` instead, where `MCP::"<backend-name>"` does
resolve correctly.
