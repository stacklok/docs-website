...
JWT claims with dot access - for example, `context.claim_act.sub` on an
[RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693) delegation `act`
claim. Nesting deeper than 10 levels is dropped rather than converted, since
claim values can originate from an unverified upstream token.
