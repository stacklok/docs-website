...
The [Registry Server](/toolhive/guides-registry/intro) only accepts this
upstream format. The CLI and UI additionally support the legacy
[ToolHive-native registry format](./registry-schema-toolhive.mdx) for their
built-in and custom file-based registries.
