...
By the end of this tutorial, you'll have a GitHub MCP server on Kubernetes
secured with Okta OIDC authentication and Cedar RBAC policies — writers get full
access while readers see only read-only tools.
...
- A Kubernetes cluster with the ToolHive Operator installed. See the
  [Kubernetes quickstart guide](./quickstart-k8s.mdx) for setup instructions.
...
- Basic familiarity with OAuth, OIDC, and JWT concepts. For background, see
...
After applying this change, the MCP server requires a valid JWT token on every
...
## Step 5: Observe tool filtering in action
...
list_commits, list_issues, merge_pull_request, ...
```
...
The proxy denies the request and returns a `403` response:
...
Sending the same request with a writer's token succeeds.
