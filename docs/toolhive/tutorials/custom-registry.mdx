...
```json title='registry.json'
{
  ...
  "servers": {
    "my-fetch": {
      ...
      "tier": "Community",
      "transport": "stdio",
      "tools": ["fetch_url", "get_page_content"],
...
After updating the file, the new server will be immediately available in
ToolHive commands.
...
```bash
thv config set-registry https://registry.mycompany.com/mcp-registry.json
```
