...
- A standalone MCP server called `my-fetch` in `data.servers`
- A group called `dev-toolkit` containing a `fetch-tool` server in `data.groups`
...
- A `title` field - the short name used in `thv run` and `thv registry info`
- A `packages` entry pointing to the container image and transport type
- ToolHive-specific metadata (status, tier, tools, permissions, args) in the
  `_meta` extension block under the publisher namespace and image identifier
...
```json title='registry.json'
{
  ...
  "data": {
    "servers": [
      {
        // ... existing my-fetch server configuration
      },
      {
        "$schema": "https://static.modelcontextprotocol.io/schemas/2025-12-11/server.schema.json",
        "name": "com.example/company-tools",
        "title": "company-tools",
        ...
        "packages": [
          {
            "registryType": "oci",
            "identifier": "registry.company.com/mcp/company-tools:v1.2.0",
        ...
        "_meta": {
          "io.modelcontextprotocol.registry/publisher-provided": {
            "io.github.stacklok": {
              "registry.company.com/mcp/company-tools:v1.2.0": {
                "status": "Active",
                "tier": "Community",
