...
:::warning[Registry format change]

As of ToolHive v0.25.0, the legacy ToolHive-native registry format used in this
tutorial is no longer accepted by `thv config set-registry-file` or
`set-registry-url`. Convert any registry file you create here to the upstream
MCP format before applying it:

```bash
thv registry convert --in registry.json --in-place
```

The conversion is lossless. To author files directly in the upstream format
instead, see the
...
Registry groups are supported in both the CLI and the ToolHive UI.

:::

### Step 2: Configure ToolHive to use your registry

First, convert the file to the upstream MCP format that ToolHive accepts:

```bash
thv registry convert --in registry.json --in-place
```

This rewrites `registry.json` in place and writes a `registry.json.bak` backup
of the original. The conversion is lossless: every ToolHive-specific field moves
to a publisher-provided extension on the corresponding upstream server entry.
...
To install the `dev-toolkit` group, click on it in the registry grid and then
click **Install group**. The installation wizard guides you through configuring
each server in the group one at a time. For more details on installing groups,
see [Use the registry](../guides-ui/registry.mdx#registry-groups).
...
- Explore the full
  [upstream registry schema reference](../reference/registry-schema-upstream.mdx)
  to understand all available configuration options for the format ToolHive now
  consumes
