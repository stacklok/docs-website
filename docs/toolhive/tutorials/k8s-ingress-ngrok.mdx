...
approach is particularly valuable for organizations that want to provide
standardized tools and resources to their developers while maintaining control
over security, access policies, and resource usage.

Once your MCP servers are accessible via HTTPS, end users can quickly connect
their AI clients using the ToolHive CLI (`thv run`) or the ToolHive UI without
needing to install or configure individual MCP servers locally. This centralized
model simplifies deployment, improves consistency, and makes it easier to manage
updates and security patches across your organization.
...
- The [ToolHive CLI](../guides-cli/install.mdx) to interact with the "remote"
  MCP server and connect it to your AI clients.
...
Get the details of the MCP server's `Service` to identify the port to expose.
The service name follows the format `mcp-<MCP_SERVER_NAME>-proxy`. If you used
the example above, the service name is `mcp-mkp-proxy` in the `toolhive-system`
namespace and the port is `8080`:
...
Take note of the service name and port number, as you'll need them in the next
step.
  ...
  rules:
    - matches:
        - path:
            ...
            value: /
      backendRefs:
        - name: mcp-mkp-proxy # Service name from Step 2
          port: 8080 # Port number from Step 2
```
...
You can also use the ToolHive CLI (or UI) to quickly connect your AI clients to
the MCP server:
...
The MKP MCP server is now available to your AI clients configured with
`thv client setup` via a secure HTTPS endpoint. In an enterprise setting, this
setup allows team members to access shared tools and resources centrally without
the need for individual MCP server installations.
