...
To achieve this, you'll use the ngrok Kubernetes Operator to create secure
tunnels to your MCP servers. This setup ensures that your MCP servers are
accessible over HTTPS, providing a secure way to interact with them.
...
[ngrok Kubernetes Operator](https://ngrok.com/docs/k8s) in your cluster using
Helm:
...
Obtain your ngrok authentication token and create an API key from the ngrok
...
Install the Gateway API CRDs and a GatewayClass resource:
...
Finally, install the ngrok Operator using Helm:
...
## Optional: Tunnel multiple MCP servers with URLRewrites

If you have multiple MCP servers and want to expose them via the same ngrok
Gateway, you can use URL rewrites in the HTTPRoute resource. Note that this
option requires a paid ngrok plan, or using some of your $5 credit that comes
with a free account.
