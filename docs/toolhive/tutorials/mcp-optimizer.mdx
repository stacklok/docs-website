...
- The ToolHive API server must be running. See
  [Step 3.1](#step-3-enable-mcp-optimizer) for details.
- Create a dedicated group for the optimizer by following
  [Step 3.2](#step-3-enable-mcp-optimizer) up to (but not including) the
  `thv run mcp-optimizer` command — you will use the Linux-specific command
  below instead.
